      ...
      <div className="max-w-4xl mx-auto space-y-8">
              ...
              <section key={day.id} className="bg-white dark:bg-stone-900 p-6 rounded-3xl shadow-sm border border-stone-100 dark:border-stone-800">
                ...
                <div className="space-y-4 mb-4 min-h-[4rem]">
                  {dayMeals.length === 0 ? (
                    ...
                    dayMeals.map(meal => (
                      <div key={meal.id} className="p-3 bg-stone-50 dark:bg-stone-950 rounded-2xl border border-stone-100 dark:border-stone-800 relative group">
                        <div className="flex justify-between items-start mb-2">
                          <p className="font-bold text-sm">
                            <span className="text-[#C5A38E] mr-2">{meal.mealType === 'LUNCH' ? '☀️ Lunch' : '🌙 Dinner'}</span>
                            {meal.recipe}
                          </p>
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-2xl z-50">
          <form action={async (formData) => { 
            ...
            <input name="ingredients" placeholder="Zutaten (optional, mit Komma getrennt: Tomaten, Hackfleisch...)" className="w-full bg-stone-800 border-none text-white text-xs px-4 py-2 rounded-xl outline-none placeholder:text-stone-500" />
          </form>
        </div>
